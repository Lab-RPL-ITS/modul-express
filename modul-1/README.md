...
customEmitter.on('response', (name, id) => { // di sini menerima argumen 'john' dan 34
...
Terdiri dari status code, status text, headers, body. Status code lebih baik dihafalkan, misalkan 200 artinya OK, 400 artinya error. Pada headers, kita dapat mengatur misalkan *content-type*, yaitu tipe konten data yang kita kirim. Pada body, contohnya kita mengirim file html yang akan dikirim sebagai response.
